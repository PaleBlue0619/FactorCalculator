...
if __name__ == "__main__":
    ...
    class FactorVisualizer:
        def __init__(self, factor_calculator: FactorCalculator):
            ...
            # 使用新的颜色方案
            self.categories = [
                {"name": "日频因子", "itemStyle": {"color": COLORS[0]}},  # #5292C5
                {"name": "分钟频因子", "itemStyle": {"color": COLORS[1]}},  # #DEA3BC
                {"name": "classFunc", "itemStyle": {"color": COLORS[2]}},  # #985C8C
                {"name": "midFunc", "itemStyle": {"color": COLORS[3]}},  # #E1646E
                {"name": "数据源", "itemStyle": {"color": COLORS[4]}}  # #ffa26f
            ]
                    ...
                    for dep in deps:
                        if dep in self.fc.factor_cfg:
                            self.links.append({
                                "source": dep,
                                "target": factor_name,
                                "lineStyle": {"color": COLORS[0], "width": 2}  # 使用日频因子颜色
                            })
                    ...
                    for func_name in self.fc.class_cfg[class_name]:
                        ...
                        for factor_name in factors:
                            self.links.append({
                                "source": class_func_id,
                                "target": factor_name,
                                "lineStyle": {"color": COLORS[2], "width": 1, "type": "dashed"}  # 使用classFunc颜色
                            })
                ...
                if cfg['dependency']['midFunc']:
                    for mid_func in cfg['dependency']['midFunc']:
                        ...
                        self.links.append({
                            "source": mid_func_id,
                            "target": factor_name,
                            "lineStyle": {"color": COLORS[3], "width": 1, "type": "dotted"}  # 使用midFunc颜色
                        })
            ...
            for factor_name, cfg in self.fc.factor_cfg.items():
                for data_path in cfg['dataPath']:
                    ...
                    self.links.append({
                        "source": data_id,
                        "target": factor_name,
                        "lineStyle": {"color": COLORS[4], "width": 1, "type": "dashed"}  # 使用数据源颜色
                    })
...
# 在主程序中调用
if __name__ == "__main__":
    from func import classFunc, shioMidFunc, shioCalFunc, varCalFunc, coinCalFunc, utilFunc
    ...
    F = FactorCalculator(session=session, config=config,
                             ...
                             func_map=get_funcMapFromImport(classFunc, shioMidFunc, shioCalFunc, varCalFunc,
                                                            coinCalFunc),
                             class_cfg=class_cfg)
